...
- **何度も繰り返し使われる情報（例：リリース名・アーティスト名・レーベル名）はテーブルを分けてID参照にする**  
...
- **選択肢が少なく、今後も増減がほぼない情報（例：format＝Album/Single/EPなど）は、テーブルを分けずにenum型やアプリの定数で管理することも検討する**  
  → JOINを減らして実装・運用を簡単にできる

---

## 【今回のBEATLESデータ例：テーブル分割たたき台（2025/10時点案）】
...
- `releases`（リリース：アルバム/シングル/EP/映画/MV等を含む。id, title, type, format, release_date, label_id等）
  - ※「アルバム」「シングル」「EP」「映画」「MV」などはtypeやformatカラムで分類
  - ※ formatはENUM型または定数で管理する案もあり
- `artists`（アーティスト：id, name など）
